...
1. **子代理参考** — `https://code.claude.com/docs/en/sub-agents` — 提取支持的 frontmatter 字段的完整列表（名称、类型、是否必需、描述）和所有内置子代理类型（名称、模型、工具、描述）。
2. **变更日志** — `https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md` — 提取最后 N 个版本条目。专门查找与代理相关的更改：新增或删除的 frontmatter 字段、新增或删除的内置代理。
...
将官方文档的内置子代理（Explore、Plan、general-purpose、Bash、statusline-setup、claude-code-guide 以及任何其他）与报告的官方代理表进行比较：
- **添加的代理**：官方文档中有但我们的表中缺少的内置代理（包括模型、工具、描述）
- **删除的代理**：我们的表中有但官方文档中不再有的代理
...
1. **外部数据摘要** — 最新的 Claude Code 版本、官方字段总数、官方代理总数
